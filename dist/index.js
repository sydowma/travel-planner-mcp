...
const ctripLowPriceCache = new Map();
const airportToCitySlug = {
    SZX: "shenzhen",
    HKG: "hongkong",
    SHA: "shanghai",
    PVG: "shanghai",
...
const weekdayIndexToKey = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"];
const weekdayCnLabel = {
    mon: "周一",
    tue: "周二",
    wed: "周三",
    thu: "周四",
...
function getRoutePriceReference(origin, destination) {
...
function toMonthStartDate(year, month) {
    return `${year}-${String(month).padStart(2, "0")}-01`;
}
function parseDotNetDateToIso(raw) {
...
async function fetchCtripLowPriceCalendar(origin, destination, year, month) {
    const response = await axios.post(ctripLowPriceApiUrl, {
...
function buildCalendarCacheKey(origin, destination, year, month) {
    return `${toCtripCityCode(origin)}-${toCtripCityCode(destination)}-${year}-${String(month).padStart(2, "0")}`;
}
function getCachedCtripLowPriceCalendar(origin, destination, year, month) {
...
async function getCalendarPriceSnapshot(origin, destination, departureDate, returnDate) {
...
function listDatesByWeekdays(year, month, weekdays) {
...
function generateMockFlights(origin, destination, departureDate, priceRef) {
    const airlines = [
        { code: "ZH", name: "深圳航空" },
        { code: "CX", name: "国泰航空" },
        { code: "JL", name: "日本航空" },
...
function buildCtripUrl(origin, destination, departureDate, returnDate) {
...
function buildCtripOneWayListUrl(origin, destination, departureDate) {
...
function normalizeDateTimeLabel(raw) {
    if (typeof raw !== "string")
        return "";
...
function buildMockFlightScheduleOptions(origin, destination, departureDate, limit) {
...
function parseScheduleOptionsFromBatchSearch(payload, limit) {
    const root = payload;
    ...
    for (const itinerary of itineraryList) {
        ...
        for (const priceItem of priceList) {
            const adultPrice = parsePriceNumber(priceItem.adultPrice);
            const adultTax = parsePriceNumber(priceItem.adultTax);
            const miseryIndex = parsePriceNumber(priceItem.miseryIndex);
    ...
    options.sort((a, b) => {
        if (a.price_cny === null && b.price_cny !== null)
            return 1;
        if (a.price_cny !== null && b.price_cny === null)
            return -1;
...
async function scrapeFlightScheduleOptions(origin, destination, departureDate, limit, options) {
...
function extractPriceCandidatesFromPayload(payload) {
    const prices = [];
    const data = payload;
    const rootData = (data.data || data.result || data);
    const possibleLists = [
        rootData.flightList,
        rootData.list,
        data.flightList,
...
async function scrapeFlightPrices(origin, destination, departureDate, returnDate, options) {
    let context = null;
    const allPrices = [];
    const navigationTimeoutMs = options?.navigationTimeoutMs ?? 30000;
    ...
    try {
        ...
        page.on("response", async (response) => {
            try {
                const url = response.url();
                const contentType = response.headers()["content-type"] || "";
                const isPotentialFlightApi = url.includes("/api/flight/") ||
...
function buildFallbackFlightSnapshot(origin, destination, departureDate, hasReturnDate) {
    const priceRef = getRoutePriceReference(origin, destination);
...
async function getFlightPriceSnapshot(origin, destination, departureDate, returnDate, options) {
...
function getSakuraSignal(destination, departureDate) {
...
// ==================== 工具1: 航班搜索 ====================
server.tool("search_flights", "搜索航班信息，支持从深圳/香港出发到全球各地的航班查询", {
...
// ==================== 工具2: 天气查询 ====================
server.tool("get_weather", "查询目的地天气信息，帮助规划出行时间", {
...
// ==================== 工具3: 汇率查询 ====================
server.tool("get_exchange_rate", "查询汇率信息，帮助计算旅行预算", {
    ...
    try {
        ...
        if (apiKey) {
            const response = await axios.get(`https://v6.exchangerate-api.com/v6/${apiKey}/pair/${from}/${to}/${amount || 1}`);
            return {
                content: [{
                        type: "text",
                        text: JSON.stringify({
...
// ==================== 工具4: 行程规划建议 ====================
server.tool("plan_trip", "根据时间和偏好生成旅行行程建议", {
...
// ==================== 工具5: 最佳出行日期分析 ====================
server.tool("analyze_best_dates", "分析指定月份的最佳出行日期，综合考虑价格、天气、人流量", {
...
// ==================== 工具6: 窗口抓取与最佳日期推荐 ====================
server.tool("scan_trip_windows", "批量抓取指定月份的短途出行日期窗口，适合周四/周五出发、4天左右行程，输出性价比和体验最佳方案", {
...
}, async ({ year, month, origins, destination, trip_days, departure_weekdays, scoring_priority, scrape, max_results, }) => {
    try {
        ...
        const snapshots = await mapWithConcurrency(windows, 3, async (window) => {
            const snapshot = await getFlightPriceSnapshot(window.origin, normalizedDestination, window.departureDate, window.returnDate, {
        ...
        const evaluated = snapshots.map((item) => {
            ...
            const recommendationScore = Math.round(priceScore * selectedWeights.price +
        ...
        const ranked = [...evaluated].sort((a, b) => b.recommendation_score - a.recommendation_score ||
            a.lowest_price_cny - b.lowest_price_cny);
        const valueRanked = [...evaluated].sort((a, b) => a.lowest_price_cny - b.lowest_price_cny ||
            b.blossom_score - a.blossom_score);
        const experienceRanked = [...evaluated].sort((a, b) => b.blossom_score - a.blossom_score ||
        ...
        for (const candidate of [...ranked.slice(0, detailFetchCount), valueRanked[0], experienceRanked[0]]) {
            if (!candidate)
                continue;
            const exists = detailCandidates.some((item) => item.origin === candidate.origin &&
        ...
        if (scrape && detailCandidates.length > 0) {
            const detailResults = await mapWithConcurrency(detailCandidates, 2, async (item) => {
            ...
            for (const item of detailResults) {
                const hasScrapedSchedule = [...item.outbound_flights, ...item.return_flights].some((flight) => flight.data_source === "抓取(携程列表页)");
        ...
        const detailAttachedCount = Array.from(scheduleDetailByWindow.values()).filter((item) => item.outbound_flights.length > 0 || item.return_flights.length > 0).length;
        const detailScrapedCount = Array.from(scheduleDetailByWindow.values()).filter((item) => !item.used_mock_schedule).length;
        const attachScheduleDetails = (item) => {
